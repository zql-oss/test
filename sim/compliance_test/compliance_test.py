...
def main():
    ...
    if (os.environ.get('SIM_TOOL') == 'uvs'):
        uvsim_cmd = [r'./uvsim']
        uvsim_cmd += ['-cov', r'stmt,cond,tgl,fsm,assert']
        uvsim_cmd += ['-covtest', os.path.splitext(os.path.basename(sys.argv[1]))[0]]
        process = subprocess.Popen(uvsim_cmd, stdout=logfile, stderr=logfile)
